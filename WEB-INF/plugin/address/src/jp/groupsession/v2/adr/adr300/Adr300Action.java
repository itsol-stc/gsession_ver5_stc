...
//        ActionForward urlForward = null;
    ...
    private String __getLogMessage(
        ...
        String msg = "";
        //アドレス閲覧･編集権限
        msg += "[" + gsMsg.getMessage("address.120") + "]";
        if (form.getAdr300MemDspKbn() == GSConstAddress.MEM_DSP_ADM) {
            msg += gsMsg.getMessage("cmn.set.the.admin");
        } else if (form.getAdr300MemDspKbn() == GSConstAddress.MEM_DSP_USR) {
            msg += gsMsg.getMessage("cmn.set.eachuser");
        }
        //アドレス閲覧･編集権限
        msg += "\r\n" + gsMsg.getMessage("cmn.reading") + " : ";
        if (form.getAdr300PermitKbn() == GSConstAddress.MEM_OP_TANTO) {
            msg += gsMsg.getMessage("address.62");
            msg += "\r\n" + gsMsg.getMessage("cmn.edit") + " : ";
            msg += gsMsg.getMessage("address.62");
        } else if (form.getAdr300PermitKbn() == GSConstAddress.MEM_OP_GROUP) {
            msg += gsMsg.getMessage("group.designation");
            msg += "\r\n" + gsMsg.getMessage("cmn.edit") + " : ";
            msg += gsMsg.getMessage("group.designation");
        } else if (form.getAdr300PermitKbn() == GSConstAddress.MEM_OP_USER) {
            msg += gsMsg.getMessage("cmn.user.specified");
            msg += "\r\n" + gsMsg.getMessage("cmn.edit") + " : ";
            msg += gsMsg.getMessage("cmn.user.specified");
        } else if (form.getAdr300PermitKbn() == GSConstAddress.MEM_OP_ALL) {
            msg += gsMsg.getMessage("cmn.no.setting.permission");
            msg += "\r\n" + gsMsg.getMessage("cmn.edit") + " : ";
            if (form.getAdr300EditKbn() == GSConstAddress.MEM_OP_TANTO) {
                msg += gsMsg.getMessage("address.62");
            } else if (form.getAdr300EditKbn() == GSConstAddress.MEM_OP_GROUP) {
                msg += gsMsg.getMessage("group.designation");
            } else if (form.getAdr300EditKbn() == GSConstAddress.MEM_OP_USER) {
                msg += gsMsg.getMessage("cmn.user.specified");
            } else if (form.getAdr300EditKbn() == GSConstAddress.MEM_OP_ALL) {
                msg += gsMsg.getMessage("cmn.no.setting.permission");
            }
        }
        return msg;
