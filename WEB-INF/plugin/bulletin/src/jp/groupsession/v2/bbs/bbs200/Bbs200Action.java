 ...
 * <br>[備  考]
 */
public class Bbs200Action extends AbstractBulletinAdminAction {
    ...
    private ActionForward __doCommit(
        ...
        if (form.getBbs200IniPostTypeKbn() == GSConstBulletin.BAC_INI_POST_TYPE_KBN_USER) {
            value = gsMsg.getMessage("cmn.set.eachuser");
        } else if (form.getBbs200IniPostTypeKbn() == GSConstBulletin.BAC_INI_POST_TYPE_KBN_ADM) {
            value = gsMsg.getMessage("cmn.set.the.admin");
            if (form.getBbs200IniPostType() == GSConstBulletin.CONTENT_TYPE_TEXT_PLAIN) {
                value = value + " -> " + "テキスト形式";
            } else if (form.getBbs200IniPostType() == GSConstBulletin.CONTENT_TYPE_TEXT_HTML) {
                value = value + " -> " + "HTML形式";
            }
        }
