 ...
 * @author JTS
 */
public class Cht080Form extends Cht020Form {
...
//        super.setHiddenParam(msgForm);
    ...
    throws SQLException {
        ...
        GSValidateNippou.validateCmnFieldText(errors, cht080keyword__,
                                        "cht080keyword",
    ...
    /**
     * <br>[機  能] 指定したチャット 特例アクセスが存在するかを判定する
     * <br>[解  説]
     * <br>[備  考]
     * @param con コネクション
     * @param chsSid チャット特例アクセスSID
     * @return true:存在する false:存在しない
     * @throws SQLException SQL実行時例外
     */
    public boolean existChtSpAccess(Connection con, int chsSid) throws SQLException {
        ChtSpaccessDao spAccessDao = new ChtSpaccessDao(con);
        ChtSpaccessModel mode = spAccessDao.select(chsSid);
        if (mode == null) {
            return false;
        }
        return mode.getChsSid() == chsSid;
    }
