...
/**
 * <br>[機  能] チャット 特例アクセス登録画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Cht090Action extends AbstractChatAdminAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Cht090Action.class);
    ...
    public ActionForward executeAction(ActionMapping map, ActionForm form,
        ...
        Cht090Form chtForm = (Cht090Form) form;

        //コマンドパラメータ取得
        String cmd = NullDefault.getString(req.getParameter("CMD"), "");
        if (cmd.equals("cht090back")) {
            //特例アクセス一覧画面へ戻る
            forward = map.findForward("cht080");

        } else if (cmd.equals("confirm")) {
            //確認画面
            forward = __doOK(map, chtForm, req, res, con);
     ...
     */
    private ActionForward __doInit(ActionMapping map, Cht090Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con) throws Exception {

        con.setAutoCommit(true);
        Cht090ParamModel paramMdl = new Cht090ParamModel();
        paramMdl.setParam(form);
        Cht090Biz biz = new Cht090Biz();
     ...
     * @throws Exception 実行時例外
     */
    private ActionForward __doOK(ActionMapping map, Cht090Form form,
            HttpServletRequest req, HttpServletResponse res, Connection con)
            throws Exception {
        ...
        return map.findForward("cht090kn");
    }
    ...
    private ActionForward __doDeleteComp(
        ...
        ChtSpaccessModel accessMdl = accessDao.select(form.getCht080editData());
        ...
        try {
            Cht090ParamModel paramMdl = new Cht090ParamModel();
            paramMdl.setParam(form);
            Cht090Biz cht090biz = new Cht090Biz();
            cht090biz.deleteAccess(con, paramMdl);
            paramMdl.setFormData(form);
            con.commit();
            commit = true;
        ...
        String logName
            = "[" + gsMsg.getMessage("schedule.sch240.04") + "]" + accessMdl.getChsName();

        ChtBiz chtBiz = new ChtBiz(con, getRequestModel(req));
        String opCode = gsMsg.getMessage("cmn.delete");
        chtBiz.outPutLog(map, opCode, GSConstLog.LEVEL_INFO, logName, getRequestModel(req), "");
    ...
    private ActionForward __setKakuninDsp(
        ...
        String msg = gsMsg.getMessage("cmn.confirm.msg.delete",
                                            new String[] {gsMsg.getMessage("cht.cht080.02")});
    ...
    private ActionForward __setKanryoDsp(
        ...
        //OKボタンクリック時遷移先
        ActionForward forwardOk = map.findForward("cht080");
        ...
        cmn999Form.setMessage(
                msgRes.getMessage("sakujo.kanryo.object",
                                gsMsg.getMessage(req, "cht.cht080.02")));
