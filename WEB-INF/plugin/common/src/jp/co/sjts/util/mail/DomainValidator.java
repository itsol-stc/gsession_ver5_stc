...
public class DomainValidator implements Serializable {
    ...
    /**
     * <br>[機  能] ドメインチェック(TLD追加対応版)
     * <br>[解  説]
     * <br>[備  考]
     * @param domain ドメイン
     * @return 判定結果
     */
    public boolean isValidPlus(String domain) {
        if (domain == null) {
            return false;
        }
        domain = unicodeToASCII(domain);
        ...
        String[] groups = domainRegex.match(domain);
        if (groups != null && groups.length > 0) {
            return isValidTldPlus(groups[0]);
        }
        return allowLocal && hostnameRegex.isValid(domain);
    }

    /**
     * <br>[機  能] トップレベルドメインチェック(TLD追加対応版)
     * <br>[解  説]
     * <br>[備  考]
     * @param tld トップレベルドメイン
     * @return 判定結果
     */
    public boolean isValidTldPlus(String tld) {
        tld = unicodeToASCII(tld);
        return isValidGenericTldPlus(tld);
    }
    ...
    public boolean isValidGenericTldPlus(String gTld) {
        final String key = chompLeadingDot(unicodeToASCII(gTld).toLowerCase(Locale.ENGLISH));
        return arrayContains(GENERIC_TLDS_PLUS, key);
    }
