...
/**
 * <br>[機  能] サイボウズLiveデータ移行 メンバー名簿インポート画面のアクションクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Man450Action extends AdminAction {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(Man450Action.class);
    ...
    public ActionForward executeAction(ActionMapping map,
        ...
        Man450Form man450Form = (Man450Form) form;

        //コマンドパラメータ取得
        String cmd = NullDefault.getString(req.getParameter("CMD"), "");

        if (cmd.equals("Man450_Import")) {
            //インポートボタン押下
            log__.debug("インポートボタン押下");
            forward = __doKakunin(map, man450Form, req, res, con);
        } else if (cmd.equals("Man450_Back")) {
            //戻るボタン押下
            log__.debug("戻るボタン押下");
            GSTemporaryPathUtil temp = GSTemporaryPathUtil.getInstance();
            temp.deleteTempPath(getRequestModel(req),
                    GSConstMain.PLUGIN_ID_MAIN, TEMP_DIRECTORY_ID);
            forward = map.findForward("450_back");
        } else {
            //初期表示
            log__.debug("初期表示処理");
            forward = __doInit(map, man450Form, req, res, con);
        }
        return forward;
    }
    ...
    private ActionForward __doDsp(
        ...
        //初期表示情報を画面にセットする
        Man450ParamModel paramMdl = new Man450ParamModel();
        paramMdl.setParam(form);
        Man450Biz biz = new Man450Biz(getRequestModel(req));
    ...
    private ActionForward __doKakunin(ActionMapping map,
        ...
        ActionForward forward = map.findForward("450_Import");
