 ...
 * @author JTS
 */
public class RingiCsvDao extends AbstractDao {

    /** Logging インスタンス */
    private static Log log__ = LogFactory.getLog(RingiCsvDao.class);
     ...
     */
    public RingiCsvDao(Connection con) {
        super(con);
    }
     ...
     * @param tplKeyList テンプレートSID一覧
     * @return テンプレートフォーム一覧(キー:稟議SID)
     * @throws SQLException SQL実行例外
     */
    public HashMap<String, ArrayList<RingiCsvFormModel>> select(RequestModel reqMdl,
                                                                List<String> tplKeyList)
                                                                        throws SQLException {

        PreparedStatement pstmt = null;
        ResultSet rs = null;
        Connection con = null;
        HashMap<String, ArrayList<RingiCsvFormModel>> ret =
                new HashMap<String, ArrayList<RingiCsvFormModel>>();
        if (tplKeyList.size() <= 0) {
            return ret;
        }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ArrayList<RingiCsvFormModel> list =
        ...
        if (tplKeyList.contains(cmnRtpKey)) {
            RngFormBuildBiz rngFormBiz = new RngFormBuildBiz(reqMdl);
            FormCell cell = rngFormBiz.createHanyouRingiNaiyo("");
            FormCell attachCell = rngFormBiz.createHanyouRingiTemp(null);
            RingiCsvFormModel cmnMdl = __getRngTemplateModel(cell, null);
            RingiCsvFormModel attachMdl = __getRngTemplateModel(attachCell, null);
            ArrayList<RingiCsvFormModel> list = new ArrayList<RingiCsvFormModel>();
            list.add(cmnMdl);
            list.add(attachMdl);
     ...
     * @throws SQLException SQL実行例外
     */
    private ArrayList<RingiCsvFormModel> __convertRngTemplateFormFromString(
            RequestModel reqMdl, int rtpSid, int rtpVer, String formStr)
                    throws SQLException {
        ArrayList<RingiCsvFormModel> ret = new ArrayList<RingiCsvFormModel>();
                ...
                for (List<FormCell> forms : formTable) {
                    for (FormCell cell : forms) {
                        ...
                        if (rngType == EnumFormModelKbn.block) {
                            ...
                            if (list != null) {
                                for (RingiCsvFormModel mdl : list) {
                            ...
                            // その他の場合
                            RingiCsvFormModel mdl = __getRngTemplateModel(cell, null);
                            if (mdl != null) {
                                mdl.setRtpSid(rtpSid);
                                mdl.setRtpSid(rtpVer);
                                mdl.setSort(sort);
     ...
     * @return created RingiCsvFormModel
     * @throws SQLException SQL実行例外
     */
    private RingiCsvFormModel __getRngTemplateModel(FormCell cell, String title) {
        ...
        RingiCsvFormModel ret = new RingiCsvFormModel();
     ...
     * @param cell フォーム要素
     * @return created RingiCsvFormModel List
     * @throws SQLException SQL実行例外
     */
    private ArrayList<RingiCsvFormModel> __getRngTemplateFormBlock(FormBuilder builder,
                                                                   FormCell cell) {
            ...
            if (formTable != null) {
                for (List<FormCell> forms : formTable) {
                    for (FormCell form : forms) {
                        RingiCsvFormModel mdl = __getRngTemplateModel(form, title);
                        if (mdl != null) {
                            ret.add(mdl);
                        }
                    }
     ...
     * @throws SQLException SQL実行例外
     */
    private ArrayList<RingiCsvFormModel> __getRngTemplateFormTable(FormBuilder builder,
                                                                   FormCell cell) {
