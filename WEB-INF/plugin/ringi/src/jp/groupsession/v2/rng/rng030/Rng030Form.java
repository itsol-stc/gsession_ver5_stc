...
public class Rng030Form extends Rng010Form {
     ...
     * @return エラー
     */
    public ActionErrors validateCheckApplicate(RequestModel reqMdl) {
        ActionErrors errors = new ActionErrors();

        //-- タイトルチェック --
        GsMessage gsMsg = new GsMessage(reqMdl);
        String msg = gsMsg.getMessage("cmn.title");
        ...
        //-- 内容チェック --
        getRng030template().validateCheck(errors, reqMdl);
        return errors;
    }
    /**
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng030InitFlg__
     */
     ...
     */
    public ActionErrors validateCheckAddKeiro(
        ...
        //承認経路未選択チェック
        Rng020EntryKeiroBiz keiroBiz = new Rng020EntryKeiroBiz(con, reqMdl, 0);
        String keiro = gsMsg.getMessage("rng.24");

        int keiroCnt = 0;
        int eCnt = errors.size();
        RngKeiroStepDao keiroDao = new RngKeiroStepDao(con);
        int rCnt = 0;
        Map<Integer, Rng020KeiroBlock> sortKeiroMap = new HashMap<Integer, Rng020KeiroBlock>();
        //追加経路を経路順に並べる
        for (Entry<Integer, Rng020KeiroBlock> entry : rng030addKeiroMap__.entrySet()) {
            int sortNo = keiroDao.getSortNoKeiro(entry.getKey(), getRngSid());
            Rng020KeiroBlock block = entry.getValue();
            //追加経路は自己審議許可
            block.getPref().setOwn(RngConst.RNG_OWNSINGI_YES);
            sortKeiroMap.put(sortNo, block);
        }
        for (Entry<Integer, Rng020KeiroBlock> entry : sortKeiroMap.entrySet()) {
            keiroCnt = entry.getKey() + rCnt;
            rCnt += keiroBiz.validateKeiro(errors, entry, keiro, keiroCnt);
        }
        if (eCnt < errors.size()) {
            return errors;
        }
        if (rCnt == 0) {
            msg = new ActionMessage("error.addkeiro.zero.ringi");
    ...
    /**
     * 経路追加処理関連パラメータのcmn999への書き出し
     * @param cmn999Form 書き出し対象
     */
    public void addHiddenParamAddKeiro(Cmn999Form cmn999Form) {
        cmn999Form.addHiddenParam("rng030AddKeiroMode", rng030AddKeiroMode__);
        for (Entry<Integer, Rng020KeiroBlock> entry:rng030addKeiroMap__.entrySet()) {
            Rng020KeiroBlock block = entry.getValue();
            int blockKey = entry.getKey();
            for (Entry<Integer, Rng020Keiro> entryKeiro:block.getKeiroMap().entrySet()) {
                Rng020Keiro keiro = entryKeiro.getValue();
                int keiroKey = entryKeiro.getKey();
                StringBuilder sb = new StringBuilder();
                sb.append("rng030addKeiro(");
                sb.append(blockKey);
                sb.append(").keiro(");
                sb.append(keiroKey);
                sb.append(").usrgrpSel.selectedSimple");
                String paramName = sb.toString();
                cmn999Form.addHiddenParam(paramName, keiro.getUsrgrpSel().getSelectedSimple());
            }
        }
    }
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#svRng130Category__
     */
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng130searchSubject3__
     */
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#svRng130searchSubject3__
     */
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#svRng130Status__
     */
     ...
     * @return rng070Kekka
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng070Kekka__
     */
     ...
     * @return rng130Status
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng130Status__
     */
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng030UseApiConnect__
     */
     ...
     * @see jp.groupsession.v2.rng.rng030.Rng030Form#rng030ApiComment__
     */
