 ...
 */
@ParamModel
public class SchEntitiesPutParamModel implements IPutParamModel {
    /** スケジュールSID */
    @NotBlank
    ...
    /**同時施設予約編集                                          */
    @Selectable({"0", "1"})
    private Integer sameFacilityReserveEditFlg__ = GSConstSchedule.SAME_EDIT_OFF;
    /**編集前 スケジュール情報*/
    ScheduleSearchModel oldData__;
    ...
    /**会社拠点ID配列                                       []         */
    private String[] clientCompanyBaseCodeArray__;
    /**顧客アドレスSID配列                                      []        */
    ...
    public String[] getClientCompanyBaseIdArray() {
        return clientCompanyBaseCodeArray__;
    }
    /* (非 Javadoc)
    ...
    public void setClientCompanyBaseIdArray(String[] clientCompanyBaseIdArray) {
        clientCompanyBaseCodeArray__ = clientCompanyBaseIdArray;
    }
    /**
    ...
    public Map<String, CmnGroupmModel> getGroupMap(RestApiContext ctx) {
        ...
        try {
            ...
            SchDataPubDao schPubDao = new SchDataPubDao(ctx.getCon());
            int[] oldGrpSids = Stream.of(
                    Optional.ofNullable(old)
                    .filter(scd -> scd.getScdUsrKbn() == GSConstSchedule.USER_KBN_GROUP)
                    .map(scd -> scd.getScdUsrSid())
                    .stream(),
                Optional.ofNullable(old)
                    .map(scd -> {
                        try {
                            return schPubDao.select(scd.getScdSid());
                    ...
                    .filter(put -> put.getSdpType() == GSConstSchedule.SDP_TYPE_GROUP)
                    .map(put -> put.getSdpPsid())
            )
            .flatMap(str -> str)
            .mapToInt(sid -> sid.intValue())
            .toArray();

            if (oldGrpSids.length == 0) {
                return groupMap__;
            }
            List<CmnGroupmModel> oldGrpList =  grpDao.selectFromSid(
                    oldGrpSids
                        );
    ...
    public Map<String, CmnUserModel> getUserMap(RestApiContext ctx) {
        ...
        try {
            ...
            ScheduleSearchModel old = getOldData(ctx);
            //同時登録スケジュールSIDリスト
            ScheduleSearchDao ssDao = new ScheduleSearchDao(ctx.getCon());

            SchDataPubDao schPubDao = new SchDataPubDao(ctx.getCon());
            List<CmnUserModel> oldUsrList =  usDao.getUsersDataList(
                        Stream.of(
                            Optional.ofNullable(old)
                                .filter(scd -> scd.getScdUsrKbn() == GSConstSchedule.USER_KBN_USER)
                                .map(scd -> scd.getScdUsrSid())
                                .stream(),
                            Optional.ofNullable(old)
                                .map(scd -> scd.getScdAuid())
                                .stream(),
                            Optional.ofNullable(old)
                                .map(scd -> scd.getScdEuid())
                                .stream(),
                            Optional.ofNullable(old)
                                .map(scd -> {
                                    try {
                                        return ssDao.getSameScheduleList(
                                                scd.getScdGrpSid(), List.of());
                                    } catch (SQLException e) {
                                        throw new RuntimeException("SQL実行時例外", e);
                                    }
                                })
                            ...
                            Optional.ofNullable(old)
                                ...
                                .stream()
                                .flatMap(lst -> lst.stream())
                                .filter(put -> put.getSdpType() == GSConstSchedule.SDP_TYPE_USER)
                                .map(put -> put.getSdpPsid())
                        )
                        .flatMap(str -> str)
            ...
            userMap__.putAll(
                      oldUsrList.stream()
                      .collect(Collectors.toMap(usr -> usr.getUsrLgid(), usr -> usr))
                    );
     ...
     * @param wornCommitFlg wornCommitFlg
     * @see #warnCommitFlg__
     */
    @Override
    public void setWarnCommitFlg(int wornCommitFlg) {
        warnCommitFlg__ = wornCommitFlg;
    }
    ...
    public void validate(RestApiContext ctx) throws SQLException {
        IPutParamModel.super.validate(ctx);
    }
