...
//        }
    ...
    private ActionForward __doCommit(ActionMapping map, Sch097Form form,
        ...
        String[] repeat = {
                ...
                gsMsg.getMessage("schedule.sch097.1")
        };
        int repeatKbn = Integer.parseInt(paramMdl.getSch097RepeatKbn());
        int repeatMyKbn = NullDefault.getInt(paramMdl.getSch097RepeatMyKbn(), 0);
        value += repeat[repeatKbn];
        if (repeatKbn == GSConstSchedule.SCH_REPEAT_KBN_NG
                && repeatMyKbn == GSConstSchedule.SCH_REPEAT_MY_KBN_OK) {
            value += " " + gsMsg.getMessage("schedule.sch097.2");
        }

        String dspName = gsMsg.getMessage("cmn.preferences2") + " "
                         + gsMsg.getMessage("schedule.108") + " "
                         + gsMsg.getMessage("schedule.147");
