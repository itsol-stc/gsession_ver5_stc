...
//    }
    ...
    public void downloadHtmlForWebmail(
        ...
        //JavaScriptの無効化を行う
        htmlBody = StringUtilHtml.removeIllegalTag(htmlBody); //<script>タグの除去と<iframe>タグの除去 ※１
        htmlBody = StringUtilHtml.removeAttribute(htmlBody, "img", "onerror"); //<img>のonerror属性の除去
        htmlBody = StringUtilHtml.removeAttribute(htmlBody, "audio", "onloadstart"); //<audio>のonloadstart属性の除去
        htmlBody = StringUtilHtml.removeAttribute(htmlBody, "video", "onloadstart"); //<video>のonloadstart属性の除去
        ...
        if (lastLenHead > 0) {
            htmlBody = htmlBody.substring(0, lastLenHead) + script + htmlBody.substring(lastLenHead);
        } else if (lastLenHtml > 0) {
            htmlBody = htmlBody.substring(0, lastLenHtml) + script + htmlBody.substring(lastLenHtml);
        }

        IOTools.writeText(saveFilePath.getPath(), charset, htmlBody);
