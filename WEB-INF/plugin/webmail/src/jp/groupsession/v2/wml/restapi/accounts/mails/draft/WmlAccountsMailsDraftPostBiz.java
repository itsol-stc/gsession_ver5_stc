...
public class WmlAccountsMailsDraftPostBiz {
    ...
    /** パラメータ */
    private WmlAccountsMailsDraftPostParamModel param__;
    ...
    */
    public WmlAccountsMailsDraftPostBiz(WmlAccountsMailsDraftPostParamModel param,
    ...
    private WmlMailSendBiz __createMailSendBiz() throws SQLException, IOToolsException, IOException, TempFileException {
        ...
        int procType = param__.getProcType();
        long sendMessageNum = param__.getRefarenceSid();
        ...
        boolean canReadFlg = mailDataBiz.canAccess(con, wacSid, param__.getRefarenceSid());

        if (!canReadFlg) {
            //参照メールを使用できない場合、新規作成として草稿保存する
            procType = GSConstWebmail.SEND_TYPE_NORMAL;
            sendMessageNum = 0;
        }

        //送信タイプが転送または複写の場合、参照メールの添付ファイルをテンポラリディレクトリに展開
        if (param__.getProcType() == GSConstWebmail.SEND_TYPE_FORWARD
            || param__.getProcType() == GSConstWebmail.SEND_TYPE_EDIT) {

            if (canReadFlg) {
                //参照メールを使用できる場合、添付ファイルをテンポラリディレクトリに配置
        ...
        if (canUseTemplate) {
            //テンプレートを使用できる場合、添付ファイルをテンポラリディレクトリに配置
            //利用できない添付ファイルは配置処理の中ではじかれる
        ...
        WmlMailSendBiz mailBiz = WmlMailSendBiz.builder()
            ...
            .setWacSid(wacSid)
            .setSendMailType(procType)
            .setSendMessageNum(sendMessageNum)
            .setHtmlMail(false)
    ...
    */
    ...
    private void __outputResultLog(SmtpSendModel sendData)  {
        ...
        wmlBiz.outPutApiLog(reqMdl, ctx__.getCon(), ctx__.getRequestUserSid(),
                "RESTAPI_MAIL_ADD_DRAFT", gsMsg.getMessage("cmn.entry"),
