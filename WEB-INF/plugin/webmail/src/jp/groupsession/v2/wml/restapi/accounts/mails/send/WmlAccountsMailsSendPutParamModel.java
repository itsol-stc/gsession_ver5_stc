 ...
 */
@ParamModel
public class WmlAccountsMailsSendPutParamModel extends WmlAccountsMailsPutParamModel {

    /** ロギングクラス */
    private static Log log__ = LogFactory.getLog(WmlAccountsMailsSendPutParamModel.class);
    ...
    public void validate(RestApiContext ctx) throws SQLException, RestApiValidateException {
        ...
        if (addressInputFlg) {
            try {
                //宛先アドレスフォーマットチェック
                __validateAddress(ctx, getToArray(), gsMsg.getMessage("cmn.from"), "toArray");
            } catch (RestApiValidateException e) {
                valErr.add(e);
            }
            ...
            try {
                //Ccアドレスフォーマットチェック
                __validateAddress(ctx, getCcArray(), gsMsg.getMessage("cmn.cc"), "ccArray");
            } catch (RestApiValidateException e) {
                valErr.add(e);
            }
            ...
            try {
                //Bccアドレスフォーマットチェック
                __validateAddress(ctx, getBccArray(), gsMsg.getMessage("cmn.bcc"), "bccArray");
            } catch (RestApiValidateException e) {
                valErr.add(e);
            }
        }        
     ...
     * @throws SQLException 
     */
    private void __validateAddress(RestApiContext ctx, String[] addressArray, String dispName, String paramName) throws SQLException {
    ...
    private void __validateRefarenceSid(RestApiContext ctx) throws SQLException {
        ...
        //受信, 送信済み, 予約送信, 草稿のメールが指定されている場合エラー
    ...
    private void __validateTemplateBinSid(RestApiContext ctx) throws SQLException {
        ...
        wtbBiz.validateBinReqTempBin(
            ctx, templateSid, getTemplateBinSidArray(), "templateBinSidArrayBinSidArray");
    }
}
