...
/**
 * <br>[機  能] 個人設定 メール情報一括削除画面のフォームクラス
 * <br>[解  説]
 * <br>[備  考]
 *
 * @author JTS
 */
public class Wml320Form extends Wml090Form {

    /** メール一覧 ソートキー 日時 */
    public static final int WML320_SKEY_DATE = 0;
    /** メール一覧 ソートキー 差出人 */
    public static final int WML320_SKEY_FROM = 1;
    /** メール一覧 ソートキー 件名 */
    public static final int WML320_SKEY_SUBJECT = 2;
    /** メール一覧 並び順 昇順 */
    public static final int WML320_ORDER_ASC = 0;
    /** メール一覧 並び順 降順 */
    public static final int WML320_ORDER_DESC = 1;
    /** 検索条件 指定する */
    public static final int WML320_DECIDE = 1;
    /** 検索条件 指定しない */
    public static final int WML320_NODECIDE = 0;
    /** 検索条件 添付無し */
    public static final int WML320_TEMPFLG_NOTHING = 2;
    ...
    /** 検索条件 ソートキー */
    private int wml320sortKey__ = WML320_SKEY_DATE;
    /** 検索条件 ソートキー */
    private int wml320order__ = WML320_ORDER_ASC;
     ...
     * @see jp.groupsession.v2.wml.wml320.Wml320Form#wml320labelCombo__
     */
     ...
     * @see jp.groupsession.v2.wml.wml320.Wml320Form#wml320svSortKey__
     */
     ...
     * @see jp.groupsession.v2.wml.wml320.Wml320Form#wml320svOrder__
     */
     ...
     * @see jp.groupsession.v2.wml.wml320.Wml320Form#wml320sortKeyCombo__
     */
    ...
    public ActionErrors validateCheck(HttpServletRequest req,
            Connection con,  RequestModel reqMdl, BaseUserModel buMdl)
        ...
        //フォルダ
        if (wml320directory__ == null || wml320directory__.length == 0) {
            String fieldFix = "directory" + ".";
        ...
        //キーワード
        if (!StringUtil.isNullZeroString(wml320keyword__)) {
        ...
        //差出人
        if (!StringUtil.isNullZeroString(wml320from__)) {
            GSValidateWebmail.validateTextBoxInput(
                    errors,
                    wml320from__,
        ...
        //送信先
        //未選択以外の場合入力チェックを行う
        if (wml320destTypeTo__ != 0
                || wml320destTypeCc__ != 0
                || wml320destTypeBcc__ != 0) {

            //不正な値が入った場合はNG
            if ((wml320destTypeTo__ != WML320_DECIDE && wml320destTypeTo__ != WML320_NODECIDE)
                    || (wml320destTypeCc__ != WML320_DECIDE
                    && wml320destTypeCc__ != WML320_NODECIDE)
                    || (wml320destTypeBcc__ != WML320_DECIDE
                    && wml320destTypeBcc__ != WML320_NODECIDE)) {
                String fieldFix = "dest" + ".";
                ActionMessage msg = new ActionMessage("cmn.select.4",
                        gsMsg.getMessage("wml.wml320.9"));
                StrutsUtil.addMessage(
                        errors, msg, fieldFix + "cmn.select.4");
            }
            //選択されている場合は未入力チェックも行う
            if (wml320destTypeTo__ == Wml010ParamModel.DESTINATION_SELECT
                    || wml320destTypeCc__ == Wml010ParamModel.DESTINATION_SELECT
                    || wml320destTypeBcc__ == Wml010ParamModel.DESTINATION_SELECT) {
                GSValidateWebmail.validateTextBoxInput(
                        errors,
                        wml320dest__,
            ...
            if (!StringUtil.isNullZeroStringSpace(wml320dest__)) {
                String fieldFix = "wml320dest.";
                ActionMessage msg =  new ActionMessage("error.select.required.text",
                        gsMsg.getMessage("wml.wml320.9"));
                StrutsUtil.addMessage(
                        errors, msg, fieldFix + "error.select.required.text");
            }
        }

        //日付
        if (String.valueOf(wml320dateType__).equals(Wml010Const.SEARCH_DATE_SET)) {
            ...
            //日付 From
            if (!StringUtil.isNullZeroString(wml320DateFr__)) {
                errors.add(picker.setYmdParam(this, "wml320DateFr",
                        "wml320dateYearFr", "wml320dateMonthFr",
                        "wml320dateDayFr", strDateNameJp));
            }
            if (errCount == errors.size()) {
                GSValidateWebmail.validateDate(
                        req,
                        errors,
            ...
            //日付 To
            if (!StringUtil.isNullZeroString(wml320DateFr__)) {
                errors.add(picker.setYmdParam(this, "wml320DateTo",
                        "wml320dateYearTo", "wml320dateMonthTo",
                        "wml320dateDayTo", endDateNameJp));
            }
            if (errCount == errors.size()) {
                GSValidateWebmail.validateDate(
                        req,
                        errors,
                        gsMsg.getMessage("wml.206"),
                        gsMsg.getMessage("wml.206"),
                        wml320dateYearTo__,
                        wml320dateMonthTo__,
                        wml320dateDayTo__,
            ...
            if (errCount == errors.size()) {
                ...
                frDate.setDate(wml320dateYearFr__, wml320dateMonthFr__, wml320dateDayFr__);
                toDate.setDate(wml320dateYearTo__, wml320dateMonthTo__, wml320dateDayTo__);

                errors = __checkDateRange(errors, gsMsg.getMessage("wml.202"),
                        gsMsg.getMessage("wml.206"), frDate, toDate);
            }
        ...
        //ラベル
        if (wml320label__ > 0) {
            checkLabel(errors, wml320label__, wml320account__,
                    gsMsg.getMessage("cmn.label"), con);

        }

        //サイズ
        if (!StringUtil.isNullZeroString(wml320size__)) {
            GSValidateWebmail.validateNumber(
                    errors,
                    wml320size__,
     ...
     * @return エラー
     */
    private ActionErrors __checkDir(ActionErrors errors, int[] directory,
            String paramName) {
        String fieldFix = paramName + ".";
        for (int dir : directory) {
            boolean numberFlg = true;
            if (numberFlg) {
                if (dir < GSConstWebmail.DIR_TYPE_RECEIVE
                        && GSConstWebmail.DIR_TYPE_STORAGE < dir
                        || GSConstWebmail.DIR_TYPE_SPAM == dir) {
                    ActionMessage msg = new ActionMessage("search.data.notfound", paramName);
                    StrutsUtil.addMessage(
                            errors, msg, fieldFix);
                }
            } else {
     ...
     * @return エラー
     */
    private ActionErrors __checkDateRange(ActionErrors errors, String nameFrom,
            String nameTo, UDate frDate, UDate toDate) {
    ...
    public boolean checkAccount(
        ...
        boolean existFlg = wmlDao.canUseAccount(wacSid, userSid);
        if (!existFlg) {
            msg = new ActionMessage("error.select.required.text", paramName);
            StrutsUtil.addMessage(
                    errors, msg, fieldFix);
        }
        return false;
    ...
    public boolean checkLabel(
        ...
        String fieldFix = paramName + ".";
        WmlLabelDao dao = new WmlLabelDao(con);
        int cnt = dao.checkLabel(account, label);
        if (cnt <= 0) {
            //存在チェック
            msg = new ActionMessage("search.data.notfound", paramName);
            StrutsUtil.addMessage(
                    errors, msg, fieldFix);
        }
        return false;
