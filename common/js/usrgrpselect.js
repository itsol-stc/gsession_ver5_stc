...
;(function($) {
    ...
    var init = function(id, keys, titles, scrollY, banUsrSid, setChange) {
        ...
        function addBtnClick() {
            var selectTr = $(this).parents('tr:first');
            var hiddens = selectTr.find('td:first');
            var name = selectTr.attr('name');
            $.each($(this).parents('table:first').find('.noSelected option:selected'),
                    function() {
        ...
        usrgrpselect.find('.js_btn_base1add').click(addBtnClick);
        //削除ボタンイベント設定
        function delBtnClick() {
            var selectTr = $(this).parents('tr:first');
            var hiddens = selectTr.find('td:first');
            var name = selectTr.attr('name');
             $.each(hiddens.find('option:selected'),
                     function() {
                 var sid = $(this).attr('value');
                 $('input[type=\"hidden\"][name=\"' + name +'\"][value=\"' + sid +'\"]').remove();
             });
             $(this).parents('table:first').change();
          }
        usrgrpselect.find('.js_btn_base1del').click(delBtnClick);
    };
    /**
     * ダイアログ表示
     */
