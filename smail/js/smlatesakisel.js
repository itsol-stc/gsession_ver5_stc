...
;(function($) {
    ...
    function openAtesakiSel(btn) {
        ...
        $(dialog).dialog({
            ...
            buttons: [{
                    text:msglist_smlatesakisel["cmn.close"],
                    click: function() {
                    $(this).dialog('close');
                }
            }]
    ...
    */
    ...
    function resetAtesakiScr(atesakiArea) {
        ...
        if (3 < atesakiArea.find('div').length) {
            ...
            if (allDisp.text() != msglist_smlatesakisel['cmn.close']) {
                atesakiScr.addClass('sendAtesakiList-scr');
                if (allDisp.text().length == 0) {
                    allDisp.text(msglist_smlatesakisel['sml.218']);
                }
            }
    ...
    */
    ...
    $(function() {
        ...
        $(document).on("click", ".js_atesakiAllDisp", function(){
            ...
            //スクロール表示だった場合
            if ($(this).text() == msglist_smlatesakisel['cmn.close']) {
                $(this).text(msglist_smlatesakisel['sml.218']);
            } else {
                $(this).text(msglist_smlatesakisel['cmn.close']);
            }
